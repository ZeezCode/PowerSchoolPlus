...
<html>
    ...
    <body>
        ...
        <div id="mainContent">
            <center><u><h1>Announcements</h1></u></center>

            <?php
                $getAnnouncementsSQL = sprintf("SELECT * FROM ps_announcements ORDER BY posttime DESC LIMIT 2;");
                $getAnnouncementsQuery = mysqli_query($dbconnect, $getAnnouncementsSQL);
                if (mysqli_num_rows($getAnnouncementsQuery) == 0) {
                    echo "<br /><p>There are presently no announcements available to you.</p>";
                } else {
                    while ($annInfo = mysqli_fetch_assoc($getAnnouncementsQuery)) {
                        echo "<br />";
                        $title = $annInfo['title'];
                        if (strlen($title) >= 40) {
                            $title = substr($annInfo['title'], 0, 39) . "...";
                        }

                        echo "<h2 class='announcementTitle'>$title</h2>\n";

                        $announcementDate = date("F jS, Y", $annInfo['posttime']);
                        echo "<span class='announcementDate'>$announcementDate</span>\n";

                        $messageText = $annInfo['message'];
                        if (strlen($messageText) >= 350) {
                            $messageText = substr($messageText, 0, 349);
                        }
                        $messageText = wordwrap($messageText, 50, "<br />\n", false);
                        echo "<p class='announcementText'>$messageText</p>";
                        echo "<hr />";
                    }
                }
            ?>
